...
// [C] Computation
        Matrix<T> computeCn(const T& thetaGn, const T& Hs, const Matrix<T>& DCM_BGn) {// [C_n] for each CMG
    ...
    Matrix<T> computeC(const Matrix<T>& thetaG, const T& Hs, const Matrix<T>& DCM_BG) {// [C] for all CMGs NOTE: this assumes DCM_BG is a [3x24] and could be inefficent :(
        Matrix<T> C(3,N);
        for (int i = 0; i < N; i++) {
